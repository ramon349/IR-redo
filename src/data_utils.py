    ...
    def __getitem__(self, idx):
        ...
        for i in paths :
            ...
            if self.transform:
                temp = self.transform(temp)
            images.append(temp)
        return (images[0],images[1],imclass)


class LTiny(Tiny): 
    def __init__(self, root_dir=None, transform=None, loader = pil_loader):
        """
        Args:
            csv_file (string): Path to the csv file with annotations.
            root_dir (string): Directory with all the images.
        ...
        self.big_dict = {}
        L = []
        dataset_paths = os.listdir(os.path.join(self.root_dir))
        dataset_paths = [e for e in dataset_paths if e != '.DS_Store']
        for i,j in enumerate(dataset_paths):
            self.class_dict[j] = i
            self.rev_dict[i] = j
            image_paths:list = os.listdir(os.path.join(self.root_dir,j))
            self.image_dict[j] = np.array(image_paths )
            for k,l in enumerate(os.listdir(os.path.join(self.root_dir,j))):
                L.append((l,i))
    ...
    def _sample(self,idx):
        im, im_class = self.big_dict[idx]
        im2 = np.random.choice(self.image_dict[self.rev_dict[im_class]])
        numbers = list(range(0,im_class)) + list(range(im_class+1,self.num_classes))
        class3 = np.random.choice(numbers)
        im3 = np.random.choice(self.image_dict[self.rev_dict[class3]])
        p1 = os.path.join(self.root_dir,self.rev_dict[im_class],im)
        p2 = os.path.join(self.root_dir,self.rev_dict[im_class],im2)
        p3 = os.path.join(self.root_dir,self.rev_dict[class3],im3)
        return[p1,p2,p3]
class single_LTiny(LTiny): 
